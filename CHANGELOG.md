...
## 3.25.0
- **`termux-api` is now a standard package, installed by `ab setup` (the Termux step).** The phone bridge — `termux-battery-status` · `termux-notification` · `termux-clipboard-get`/`-set` · `termux-toast` · `termux-share` — is the one thing an agent on a phone can't reach any other way: a box is a Debian rootfs with no Android in it, and Termux itself only ships `termux-wake-lock` (that one comes from `termux-tools`, which is why anti-sleep has always worked without this). It joins `openssh termux-services iproute2 procps proot-distro` on the same `pkg install` line, so a fresh phone comes up with the bridge already there instead of the first `termux-notification` failing with "command not found".
  - **The CLI is only half of it — apt cannot install the other half.** The `termux-api` package is a set of thin wrappers that talk to the **Termux:API app**, a separate APK. Without it the wrappers install fine and every call fails, so the step now says so in yellow the moment it finishes, including the part that bites: the app must come from the **same source as Termux** (F-Droid or Play). A mixed pair is signature-blocked by Android and simply won't install.
  - **Host-side only.** `$PREFIX/bin` isn't on a box's `PATH`, so `termux-*` runs from a host shell (`ab host-<name>`) or from `ab` itself, not from inside a box.
  - **Existing installs:** `ab update` refreshes code only and never re-walks setup, so run `ab setup` and take the "set up Termux" step to pick it up. The step is idempotent — it re-runs the same `pkg install` and changes nothing else.
- Not mirrored to the Ubuntu Touch build (`agentbox-ut`) yet — it has no `initphone`, so there is nothing to mirror.

## 3.24.1
- **The two tmux macro keys say `hide` and `copy`, not `det` and `scr`.** Both old labels named tmux's vocabulary instead of what the key does to your screen — and `scr` was the worse of the two, because it promised scrolling while `Ctrl-b [` actually puts you in **copy-mode**, which captures the arrows and only lets go on `q`/`ESC`; a key that hides the mode you're now stuck in is the wrong label for a row whose entire purpose is to spare you the chord. `hide` says the session is out of sight and still running (`ab session` brings it back), `copy` names the mode you're in. Four characters fit — `CTRL` sits in the same row of eight and is wider — so neither has to stay an abbreviation. Labels only: the macros, the row layout, and the key count are unchanged. Applied by `ab setup`; a `termux.properties` that's already deployed keeps the old labels until you re-run it.

## 3.24.0
- **`ab <name> bash` now opens a session, like `ab <name>` always has.** A cmd used to force the foreground: the auto-session fired only when no cmd was given, so `ab claude bash` — the way you get a shell in a box — dropped you into a bare proot login that an ssh drop or an app switch killed, while the identical `ab claude` survived both. The session is now opened for **any cmd you sit in**, judged on its first word: a shell (`bash` · `sh` · `zsh` · `fish` · `dash` · `ash`), a CLI agent (`claude` · `codex` · `copilot`), or a REPL (`node` · `python` · `python3` · `irb`).
  - **Anything else still runs in the foreground, on purpose.** A tmux pane restores the screen when it exits, so a one-shot in a session loses exactly what you ran it for — `ab claude "npm test"` keeps its output where you can read it. The one-shot form of an interactive program counts as one-shot: `claude -p …` · `codex exec …` · `copilot --prompt …` · `bash -c …` · `node -e …`. The test is made **per family**, because the flags collide — `bash -c` is a one-shot while `claude -c` resumes a session.
  - `AB_SESSION=1` forces a session for any cmd; `AB_NO_SESSION=1` still disables it entirely. Piped output, an existing `$TMUX`, and desktop boxes are unchanged. Applies to `host-*` shells too.
- **Removed the `run` template.** It was a claude box with `RUN=""` — "the agent is installed, you land in a shell" — which `ab claude bash` now does to any box, without a second box to build, feed tokens to, and keep upgraded. It was also the one shipped name that its own shorthand couldn't reach: `run` is a command, so `ab run` never meant that box. `ab update` prunes the shipped copy; a copy you edited in `~/.config/agentbox/profile/` is never touched (delete it with `ab remove run`, or keep it — it still runs as `ab run run`). Nothing else changed: `RUN=""` in any profile still means "open a shell", and the help screen still says so.
- **Fixed: a box whose name is a reserved command dispatched as that command inside its own session.** The session pane re-ran `ab <name>`, so a box called `ps` or `stop` opened a pane that printed the process table (or stopped a desktop) and died. The pane now re-runs `ab run <name>`, the same explicit form the help screen tells you to use.
- **`ab claude --continue` instead of `ab claude claude --continue`.** A cmd still replaces the profile's `RUN`, but a cmd that **starts with a flag is added to it** — a flag can't be a program name, so the two cases never collide. The flags are `%q`-quoted per argument (the replace case stays a flat string), so `ab claude -p "read this repo"` reaches the agent as one argument rather than three. A profile with no `RUN` says so instead of running a bare flag; `host-*` shells, which have no `RUN` at all, say it too.
- **Help now wraps.** Every ▍Notes line is a whole rule and several run 200+ characters; on a phone they wrapped back to column 0 and the block stopped reading as a list. Notes, command rows, and section hints are now word-wrapped with a hang indent under their own column, measured with the existing `dw()` so `·`/`→`/`—` cost the one cell they print. Narrow screens put a note's text on its own lines under the label, matching what `row()` already did.
- **Help:** the Boxes block states that a cmd replaces the profile's `RUN`, names `ab <name> bash` as the shell and `ab claude --continue` as the flag form; three notes (`shell`, `flags`, `session`) spell out the rules.
- Not mirrored to the Ubuntu Touch build (`agentbox-ut`) yet.

## 3.23.0
- **`ab pkg <box> search <term>` now finishes the job it starts.** It used to print names and stop, leaving the two steps that matter — putting the package in `PKGS`, then applying it — to hand-typing. It now picks the package the term points at (exact name > `python3-<term>` > shortest name containing it, so `-doc`/`-dev` lose), **adds what belongs beside it but is not a dependency of it** — a `python3-*` library depends on `python3` and never on pip, so `python3-openpyxl` alone leaves a box that holds openpyxl and still can't `pip install` the next one — and offers to write the set into `PKGS` and apply it in place with `ab upgrade`. Enter does it; `n` prints the command and changes nothing.
  - Before asking, it says what each package **is in that box right now**, read from apt's own `policy` (`new` · `a → b upgrade` · `current`) with the real size of the install from `apt-get install -s` (dependencies included) — not a guess.
  - **Already installed is not a reason to go quiet.** An installed package with a newer candidate is shown as an upgrade and still prompts. The only silent ending is "already in PKGS and current".
  - Companions are named per family and kept small — only where they're certain: `python3-*` → `python3 python3-pip`, `ruby-*` → `ruby`, `php-*` → `php-cli`. Everything else is offered on its own.
  - Guardrails: with no terminal to ask on it prints the command and mutates nothing (a search inside a script must not silently upgrade a box; `AB_YES=1` is the explicit opt-in); a docker-mode profile (`INSTALL` set) is told to edit `INSTALL=` instead, since an offer to write `PKGS` there would promise something the build ignores; a companion this box's apt doesn't know is dropped from the set with a red row rather than failing the install later; and no match now says so, including that the package may be pip/npm-only or the lists stale.
- **Fixed wording:** `ab pkg <box> add` pointed at `ab remove <box> && ab <box>` — a full re-download — as the way to apply a `PKGS` change. It now points at `ab upgrade <box>` (in place, nothing re-downloaded), with `ab rebuild` named as the clean-slate option, matching what `agentbox-ut` already said.
- Not mirrored to the Ubuntu Touch build (`agentbox-ut`) yet.
